...
void AVSynchronizer::OnInitFromUploaderGLContext(EGLCore *eglCore, int videoFrameWidth,
...
FrameTexture *AVSynchronizer::getCorrectRenderTexture(bool forceGetFrame) {
    ...
    while (true) {
        ...
        if (ret > 0) {
            ...
            const float delta =
                    (moviePosition - DEFAULT_AUDIO_BUFFER_DURATION_IN_SECS) - texture->position;
...
void AVSynchronizer::signalDecodeThread() {
    ...
    if (!isDestroyed && (decoder->hasSeekReq()) ||
        ((!isDecodingFrames) && isBufferedDurationDecreasedToMin)) {
